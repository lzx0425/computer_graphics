...
Display( )
{
	...
	float earth_x = 20 * sin(theta);
	float earth_y = 20 * cos(theta);
	float moon_x = earth_x + 1 * sin(theta * 5);
	float moon_y = earth_y + 1 * cos(theta * 5);
	//earth
	...
	if (changeview == 0) {
		...
		gluLookAt((20.2 * sin(theta)), 0., (20.2 * cos(theta)), 
					(20 * sin(theta)), 0., (20 * cos(theta)),
					0., 1., 0.);
	}
	//moon
	else if (changeview == 2) {
		gluLookAt(1.04*moon_x, 0., 1.04*moon_y,
					0., 0., 0.,		
					0., 1., 0.);
	}
	//sun
	else if (changeview == 3) {
		gluLookAt(0., 0., 16., 
					0., 0., 0., 
					0., 1., 0.);
	}
...
#define RAD 7.f
	...
	glPushMatrix();
		...
		glPushMatrix();
			...
			glRotated(36 * Time, 0, 1, 0);
			OsuSphere(10.8, slices, stacks);
	...
	glPushMatrix();
		SetMaterial(0.1f, 1.f, 0.1f, 16.f);
		glTranslatef((float)(earth_x), 0., (float)(earth_y));
		glShadeModel(GL_SMOOTH);
		glEnable(GL_TEXTURE_2D);
		glBindTexture(GL_TEXTURE_2D, earth_tex);
		glRotated(-5 * Time, 0, 1, 0);
		OsuSphere(0.1, slices, stacks);
		glDisable(GL_TEXTURE_2D);
	glPopMatrix();
	...
	//float moon_y = earth_y + 1 * cos(theta*5);
	glPushMatrix();
		SetMaterial(1, 1, 1, 16.f);
		//glTranslatef(6., 0., 0.);
		glTranslatef((float)(moon_x), 0., (float)(moon_y));
		glShadeModel(GL_SMOOTH);
		glEnable(GL_TEXTURE_2D);
		glBindTexture(GL_TEXTURE_2D, moon_tex);
		glRotated(20 * Time, 0, 1, 0);
		OsuSphere(0.4, slices, stacks);
		glDisable(GL_TEXTURE_2D);
	glPopMatrix();
...
InitGraphics( )
{
	...
	Texture = BmpToTexture("2k_sun.bmp", &width, &height);
	...
	width = 2048;
	height = 1024;
	Texture = BmpToTexture("2k_moon.bmp", &width, &height);
	glPixelStorei(GL_UNPACK_ALIGNMENT, 1);
	...
	width = 2048;
	height = 1024;
	Texture = BmpToTexture("2k_earth_daymap.bmp", &width, &height);
	glPixelStorei(GL_UNPACK_ALIGNMENT, 1);
